...
usage: factory.py [-h] [--root ROOT] [--db DB] [--config CONFIG]
                  {config,init,status,dashboard,agent,event,baton,verify,review,pause,resume,lock,events,verification,render-ledger,doctor} ...
...
positional arguments:
  {snapshot,serve}
    ...
    serve           Serve the optional local dashboard UI.
...
usage: factory.py dashboard serve [-h] [--host HOST] [--port PORT] [--recent RECENT]
                                  [--token TOKEN] [--open] [--enable-control] [--allow-remote]
...
options:
  ...
  --enable-control  Enable control endpoints such as dashboard message requests.
